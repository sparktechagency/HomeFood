...
import { useGetsellerFooditemsQuery } from "@/redux/features/Seller/SellerApi";
import { PlusIcon, Frown } from "lucide-react";
import Link from "next/link";
import React from "react";
...
export default function Page() {
  const [page, setPage] = React.useState(1);
  const per_page = 8;
  const { data: response, isLoading, isError, refetch } = useGetsellerFooditemsQuery({ page, perPage: per_page });
  const foodData = response?.data;
  const foodItems = foodData?.data || [];
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center text-center py-20">
        ...
        <h3 className="text-xl font-semibold text-red-600">Failed to Load Items</h3>
        <p className="text-gray-500 mt-2">There was an error fetching your food items. Please try again.</p>
